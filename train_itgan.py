import warnings, random, copy, os, sys, datetime, argparse, json
...
class AEGANSynthesizer(BaseSynthesizer):
    def __init__(self, rtol, atol, batch_size, epochs, random_num, GPU_NUM, save_loc, save_arg, data_name, test_name,
                G_model, embedding_dim, G_args, G_lr, G_beta, G_l2scale, G_l1scale, G_learning_term, oG_learning_term,
                ...
                oD_model, odis_dim, olambda_grad, oD_lr, oD_beta, oD_l2scale, oD_learning_term, oD_leaky, oD_dropout,
        ...
        self.G_l1scale = G_l1scale ; self.G_learning_term = G_learning_term; self.oG_learning_term = oG_learning_term
        ...
        self.oD_model=oD_model; self.odis_dim=odis_dim; self.olambda_grad=olambda_grad; self.oD_lr=oD_lr; self.oD_beta=oD_beta
        self.oD_l2scale=oD_l2scale ; self.oD_learning_term = oD_learning_term ; self.oD_leaky = oD_leaky ; self.oD_dropout = oD_dropout
    ...
    def fit(self, train_data, test_data, meta_data, dataset_name, categorical_columns, ordinal_columns):
        ...
        if self.oD_learning_term != 0:
            self.odiscriminator = self.oD_model(data_dim, self.odis_dim, self.oD_leaky, self.oD_dropout).to(self.device)
            optimizeroD = optim.Adam(self.odiscriminator.parameters(), lr = self.oD_lr, betas= self.oD_beta, weight_decay = self.oD_l2scale)
        ...
        for i in range(self.epochs):
            for id_, data in enumerate(loader): 
                ...
                if term_check(self.ae_learning_term, iter):
                    emb, _, _ = encoder(real)
                    rec, sigmas = self.decoder(emb)
                    loss_1, loss_2 = self.L_func(
                        rec, real, sigmas, emb, 0, self.transformer.output_info, self.loss_factor)
                ...
                if term_check(self.ae_learning_term_g, iter):
                    ...
                    fakez = torch.normal(mean=mean_z, std=std_z)
                    fake_h = self.generator(fakez)
                    real_h, _, _ = encoder(real)
                    rec_real, sigmas_real = self.decoder(fake_h)
                    loss_ae_g2 = 0
                    if self.D_learning_term != 0:
                        loss_ae_g2 += -torch.mean(self.discriminator(real_h))
                    if self.oD_learning_term != 0:
                        loss_ae_g2 += -torch.mean(self.odiscriminator(rec_real))
                    optimizerEn.zero_grad()
                    loss_ae_g2.backward()
                    optimizerEn.step()
                ...
                ######## update outer discriminator #########
                if term_check(self.oD_learning_term, iter):
                    fakez = torch.normal(mean=mean_z, std=std_z)
                    fake_h = self.generator(fakez)
                    rec_syn, sigmas_syn = self.decoder(fake_h)
                    y_fake = self.odiscriminator(rec_syn)
                    y_real = self.odiscriminator(real)
                    loss_d = -(torch.mean(y_real) - torch.mean(y_fake))
                    pen = calc_gradient_penalty(self.odiscriminator, real, rec_syn, self.device, self.olambda_grad)                
                    loss_d = loss_d + pen 
                    optimizeroD.zero_grad()
                    optimizerDe.zero_grad()
                    loss_d.backward()
                    optimizeroD.step()
                    optimizerDe.step()
                    self.decoder.sigma.data.clamp_(0.01, 1.0)
                    self.writer.add_scalar('losses/D2_loss', loss_d, iter_s.oD_iter)
                    iter_s.oD_iter += 1
                ...
                ######### update generator with outer discri W-GAN Loss ##########
                if self.oD_learning_term != 0 and term_check(self.oG_learning_term, iter):
                    fakez = torch.normal(mean=mean_z, std=std_z)
                    fake_h = self.generator(fakez)
                    rec_syn, sigmas_syn = self.decoder(fake_h)
                    y_fake = self.odiscriminator(rec_syn)
                    ...
                    (loss_g + reg_g).backward()
                    optimizerG.step()
                    self.writer.add_scalar('losses/G2_loss', loss_g, iter_s.G_iter)
                    iter_s.G_iter += 1
...
if __name__ == "__main__":
    ...
    rtol = 1e-3 ; atol = 1e-3; batch_size = 2000 ; epochs = 1000 ; random_num = 777 ; GPU_NUM = 0 ; save_loc= "last_result"
    G_model= Generator; embedding_dim= 128; G_lr= 2e-4; G_beta= (0.5, 0.9); G_l2scale= 1e-6 ; G_l1scale = 0 ; G_learning_term = 3 ; oG_learning_term = 3 ; 
    ...
    oD_model = Discriminator; odis_dim= (256, 256); olambda_grad= 10; oD_lr= 2e-4; oD_beta= (0.5, 0.9); oD_l2scale= 1e-6 
    oD_learning_term = 0 ; oD_leaky = 0.2 ; oD_dropout = 0.5
    ...
    arg = {"rtol":rtol,
            ...
            "oG_learning_term": oG_learning_term,
            ...
            "oD_model":oD_model,
            "odis_dim":odis_dim,
            "olambda_grad":olambda_grad,
            "oD_lr": oD_lr,
            "oD_beta": oD_beta,
            "oD_l2scale": oD_l2scale,
            'oD_learning_term' : oD_learning_term,
            'oD_leaky' : oD_leaky,
