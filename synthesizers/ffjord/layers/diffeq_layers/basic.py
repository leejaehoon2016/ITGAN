...
class SimBlendDiv1(nn.Module):
    def __init__(self, dim_in, dim_out, layer_type=nn.Linear, **unused_kwargs):
        super(SimBlendDiv1, self).__init__()
        self._layer0 = layer_type(dim_in, dim_out)
        self._layer1 = layer_type(dim_in, dim_out)
        self._layert = nn.Sequential(nn.Linear(dim_in + 1, 1), nn.Sigmoid())
    def forward(self, t, x):
        y0 = self._layer0(x)
        y1 = self._layer1(x)
        tt = torch.ones_like(x[:, :1]) * t
        ttx = torch.cat([tt, x], 1)
        t_out = self._layert(ttx)
        return y0 + (y1 - y0) * t_out

class SimBlendDiv2(nn.Module):
    def __init__(self, dim_in, dim_out, layer_type=nn.Linear, **unused_kwargs):
        super(BlendDiv2, self).__init__()
        self._layer0 = layer_type(dim_in, dim_out)
        self._layer1 = layer_type(dim_in, dim_out)
        self._layer2 = layer_type(dim_in, dim_out)
        self._layert1 = nn.Sequential(nn.Linear(dim_in + 1, 1), nn.Sigmoid())
        self._layert2 = nn.Sequential(nn.Linear(dim_in + 1, 1), nn.Sigmoid())
    def forward(self, t, x):
        y0 = self._layer0(x)
        y1 = self._layer1(x)
        y2 = self._layer2(x)
        tt = torch.ones_like(x[:, :1]) * t
        ttx = torch.cat([tt, x], 1)
        t1 = self._layert1(ttx)
        t2 = self._layert2(ttx)
        return 2 * y0 + (y1 - y0) * t1 + (y2 - y0) * t2



class BlendDiv1(nn.Module):
    def __init__(self, dim_in, dim_out, layer_type=nn.Linear, **unused_kwargs):
        super(BlendDiv1, self).__init__()
        self._layer0 = layer_type(dim_in, dim_out)
        self._layer1 = layer_type(dim_in, dim_out)
        seq = TNetwork(dim_in)
        self._layert = nn.Sequential(*seq)
    def forward(self, t, x):
        y0 = self._layer0(x)
        y1 = self._layer1(x)
        tt = torch.ones_like(x[:, :1]) * t
...
class BlendDiv2(nn.Module):
    def __init__(self, dim_in, dim_out, layer_type=nn.Linear, **unused_kwargs):
        ...
        self._layer2 = layer_type(dim_in, dim_out)
        seq = TNetwork(dim_in)
        self._layert1 = nn.Sequential(*seq)
        seq = TNetwork(dim_in)
        self._layert2 = nn.Sequential(*seq)
    def forward(self, t, x):
        y0 = self._layer0(x)
        y1 = self._layer1(x)
        y2 = self._layer2(x)
