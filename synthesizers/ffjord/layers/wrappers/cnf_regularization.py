...
class RegularizedODEfunc(nn.Module):
    ...
    def forward(self, t, state):
        ...
        with torch.enable_grad():
            ...
            if len(state) > 2:
                dx, dlogp = dstate[:2]
                reg_states = tuple(reg_fn(x, logp, dx, dlogp, SharedContext, self.odefunc, t) for reg_fn in self.regularization_fns)
...
def l1_regularzation_fn(x, logp, dx, dlogp, unused_contextm, unused, unsed_t):
    del x, logp, dlogp
    return torch.mean(torch.abs(dx))


def l2_regularzation_fn(x, logp, dx, dlogp, unused_context, unused, unsed_t):
    del x, logp, dlogp
    return _batch_root_mean_squared(dx)


def directional_l2_regularization_fn(x, logp, dx, dlogp, unused_context, unused, unsed_t):
    del logp, dlogp
    directional_dx = torch.autograd.grad(dx, x, dx, create_graph=True)[0]
    return _batch_root_mean_squared(directional_dx)


def jacobian_frobenius_regularization_fn1(x, logp, dx, dlogp, context, unused, unsed_t):
    del logp, dlogp
...
def jacobian_diag_frobenius_regularization_fn(x, logp, dx, dlogp, context, unused, unsed_t):
    del logp, dlogp
    if hasattr(context, "jac"):
        jac = context.jac
    else:
...
def jacobian_offdiag_frobenius_regularization_fn(x, logp, dx, dlogp, context, unused, unsed_t):
    del logp, dlogp
    if hasattr(context, "jac"):
        jac = context.jac
    else:
...
def total_derivative(x, logp, dx, dlogp, unsed, unused_context, t):
...
def directional_derivative(x, logp, dx, dlogp, unsed, unused_context, t):
...
def quadratic_cost(x, logp, dx, dlogp, unsed, unused_context, t):
    del x, logp, dlogp, t, unused_context
    dx = dx.view(dx.shape[0], -1)
    return 0.5*dx.pow(2).mean(dim=-1)

def jacobian_frobenius_regularization_fn2(x, logp, dx, dlogp, unsed, context, t):
