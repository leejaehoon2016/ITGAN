...
class argument:
    def __init__(self, args, data_dims):
        ...
        if self.layer_type == "blend":
            if not (self.time_length == 1.0 and self.train_T == False):
                raise Exception("!! Setting time_length from None to 1.0 due to use of Blend layers.")
...
class Generator(nn.Module):
    def __init__(self, arg, data_dim):
        ...
        if (group1 > 0) and (group2 > 0):
            raise Exception("regularizer group should be selected once")
        elif (group1 > 0):
            self.mode = 1
        else:
            self.mode = 2
